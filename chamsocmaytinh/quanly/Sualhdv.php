...
if (isset($_GET['malhdv']) && is_numeric($_GET['malhdv'])) {
    ...
    if ($_SERVER['REQUEST_METHOD']=== 'POST') {
        ...
        $stmt = $conn->prepare(" UPDATE lichhendichvu SET malh= ?, madv=?, soluongdv=?, manv=? WHERE malhdv = ?");
        $stmt->bind_param("ssssi", $malh, $madv, $soluongdv, $manv, $malhdv);
    ...
    $sql="SELECT * FROM lichhendichvu WHERE malhdv=?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $malhdv);
    $stmt->execute();
    $result = $stmt->get_result();
    $lhdv= $result->fetch_assoc();
    $stmt->close();
}else{
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <!-- Page Heading -->
                    <h1 class="h3 mb-2 text-gray-800">Sửa thông tin lịch hẹn dịch vụ</h1>
                    ...
                    <div class="card shadow mb-4">
                        ...
                        <form method="POST" action="">
                            ...
                            <input type="text" name="malh" value="<?= $lhdv['malh'] ?>" required><br>

                            <label for="madv">Mã dịch vụ:</label>
                            <input type="text" name="madv" value="<?= $lhdv['madv'] ?>" required><br>

                            <label for="soluongdv">Số lượng dịch vụ:</label>
                            <input type="text" name="soluongdv" value="<?= $lhdv['soluongdv'] ?>" required><br>

                            <label for="manv">Mã nhân viên:</label>
                            <input type="text" name="manv" value="<?= $lhdv['manv'] ?>" required><br>
